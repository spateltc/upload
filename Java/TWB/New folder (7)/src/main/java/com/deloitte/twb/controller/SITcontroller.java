...
*/
	...
	/**
	 * Author: Shasahank Description: Rest API to get SIT_TEST_CASES Details
	 * params : planName,testType1,status
	 */

	@RequestMapping(value = "/ords/preprod/getSitTestCaseDetails", method = RequestMethod.GET)
	@ResponseBody
	public List getSitTestCaseDetails() {
		List resultList = new ArrayList();
		/*
		 * List returningResultList = new ArrayList(); Object[] totallist = new
		 * Object[5]; Object[] totallistObjects = new Object[6];
		 */
		List<PTestInstanceFinal> popupPiechartDetails = new<PTestInstanceFinal> ArrayList();
		log.debug("barChartController::getSitTestCaseDetails");
		try {
			resultList = barchartService.getSitTestCaseDetails();

			/*
			 * if (resultList != null) { for (int i = 0; i < resultList.size();
			 * i++) { totallistObjects = (Object[]) resultList.get(i); String
			 * Project = (String) totallistObjects[0]; Long Total = (Long)
			 * totallistObjects[1]; if (Total == null || Total == 0) { Total =
			 * (long) 1; } Long Executed = (Long) totallistObjects[2]; int
			 * ExecutedPercentage = (int) Math.ceil(Executed * 100.0 / Total);
			 * Long Passed = (Long) totallistObjects[3]; int PassedPercentage =
			 * (int) Math.ceil(Passed * 100.0 / Total); Long Failed = (Long)
			 * totallistObjects[4]; int FailedPercentage = (int)
			 * Math.ceil(Failed * 100.0 / Total); Long Blocked = (Long)
			 * totallistObjects[5]; int BlockedPercentage = (int)
			 * Math.ceil(Blocked * 100.0 / Total);
			 * 
			 * Object[] totallistObjectsa = new Object[10]; totallistObjectsa[0]
			 * = Project; totallistObjectsa[1] = Total; totallistObjectsa[2] =
			 * Executed; totallistObjectsa[3] = ExecutedPercentage;
			 * totallistObjectsa[4] = Passed; totallistObjectsa[5] =
			 * PassedPercentage; totallistObjectsa[6] = Failed;
			 * totallistObjectsa[7] = FailedPercentage; totallistObjectsa[8] =
			 * Blocked; totallistObjectsa[9] = BlockedPercentage;
			 * returningResultList.add(totallistObjectsa); } }
			 */
